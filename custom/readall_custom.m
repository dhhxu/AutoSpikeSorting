function [allts] = readall_custom(StartingFileName)
% READALL_CUSTOM reads spike timestamp data from .plx file to a cell structure.
%
% ALLTS = READALL_CUSTOM(STARTINGFILENAME)
%
% Requires Plexon Offline SDK for Matlab.
%
% This is a modified version of the `readall.m` script in the Plexon Offline
% SDK. Specifically, the original code has been changed from a script to a
% function, as well as cleaned up stylistically.
%
% Given a path to the .plx file, STARTINGFILENAME, reads it and converts it into
% a cell structure containing spike timestamps. If STARTINGFILENAME is an empty
% string, brings up the file-open dialog and prompt user to specify the path to
% the .plx file.
...
[OpenedFileName, Version, Freq, Comment, Trodalness, NPW, PreThresh, ...
 ...
 plx_information(StartingFileName);
